...
# INFO lines during stack phase (no tqdm: stderr is shared with loguru and breaks formatting)
...
def run_scan_cycle(settings: Settings, api: ImmichApiClient) -> ScanStats:
    ...
    for n, (key, s) in enumerate(group_items, start=1):
        if not s.stackable():
            ...
            if st is not None and st.assetCount > 0:
                ...
                if settings.read_only or settings.dry_run:
                    logger.debug(
                        "Would create stack for {!r} ({} assets) read_only={} dry_run={}",
                        key,
                        len(ordered_ids),
                        settings.read_only,
                        settings.dry_run,
